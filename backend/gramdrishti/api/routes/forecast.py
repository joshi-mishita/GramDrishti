...
PROVISIONAL: values come from the corrected block forecast B1 (see gramdrishti/provisional/forecast.py).
"""
...
@router.get("/forecast/map", response_model=s.ForecastMap, responses=ERRORS)
...
@router.get("/forecast/panchayat/{panchayat_id}", response_model=s.PanchayatForecast, responses=ERRORS)
...
def explain(svc: Svc, panchayat_id: str, issue_date: Issue, lead_day: Lead, var: s.Var) -> s.Explain:
    """Top reasons this Panchayat differs from its block (placeholder static contrast until SHAP)."""
    return svc.explain(panchayat_id, issue_date, lead_day, var)


@router.get("/forecast/changes/{panchayat_id}", response_model=s.ForecastChanges, responses=ERRORS)
